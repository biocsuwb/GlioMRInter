...
output_folder = "D:/Magisterka/Dane_LGG"

# odczytanie pliku Excel z klasami
class_df = pd.read_excel("E:/Magisterka/AllIDs.xlsx", header=None, names=["ID", "Class"])
...
for cancer_type in os.listdir(input_folder):
    if(cancer_type == "TCGA-GBM"): continue
    print(cancer_type)
...
for patient_id in os.listdir(input_folder):
    ...
    if not class_row.empty:
        ...
        os.makedirs(output_path, exist_ok=True)
        # iteracja po plikach .dcm w podfolderze pacjenta
        for root, dirs, files in os.walk(os.path.join(input_folder, patient_id)):
            for filename in files:
                print(f'Weryfikuję plik {filename}...')
                if filename.endswith(".dcm"):
                    # skopiowanie pliku .dcm do katalogu wynikowego
