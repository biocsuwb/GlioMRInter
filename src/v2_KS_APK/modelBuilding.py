...
class ModelBuilder:
    ...
    def cross_validate(self, n_repeats=10):
        if(self.skip): return
...
class ImageModelBuilding:
    ...
    def cross_validate(self, patient_ids, metrics_list=['accuracy', 'precision', 'recall', 'f1_score', 'roc_auc_score', 'mcc', 'mean_squared_error']):
...
class OmicsModelBuilding(ModelBuilder):
    ...
    def train_and_evaluate(self, model_type='random_forest', metrics_list=['accuracy', 'precision', 'recall', 'f1_score', 'roc_auc_score', 'mcc', 'mean_squared_error'], return_probabilities=False):
